...
3. **构建安装程序**
   ```bash
   "C:\Program Files (x86)\NSIS\makensis.exe" setup_http.nsi
   ```
   - 输出：`宽带拨号助手_Setup.exe`
...
dist\
├── 宽带拨号助手.exe           # 主程序（~15MB）
└── TPLinkCleanupService.exe  # 服务程序（~8MB）

宽带拨号助手_Setup.exe         # 安装程序（~25MB）
```
