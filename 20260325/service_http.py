...
class TPLinkCleanupServiceHTTP(win32serviceutil.ServiceFramework):
    ...
    _svc_name_ = "TPLinkCleanupHTTP2"
    _svc_display_name_ = "TP-Link路由器清理服务(HTTP v2)"
    _svc_description_ = "关机自动清除路由器账号（HTTP API版本v2）"
    ...
    def SvcDoRun(self):
        try:
            # 先设置日志
            self.setup_logging()
            self.logger.info("=== SvcDoRun 开始 ===")

            # ✅ 关键：立即RUNNING
            self.ReportServiceStatus(win32service.SERVICE_RUNNING)
            self.logger.info("报告状态: RUNNING")
