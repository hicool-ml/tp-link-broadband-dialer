; TP-Link Broadband Dialer - Setup Script (Shutdown Only)
; 只在关机时清除，重启时不清除
...
Section "MainSection" SEC01
  ...
  ; ✅ 方案1: 只在关机时清除（使用EventID 1074 + 过滤重启）
  ; 注意: 这需要额外的PowerShell脚本来区分关机和重启
  DetailPrint "Creating scheduled task for shutdown cleanup..."
  ExecWait 'schtasks /Delete /TN "TPLinkCleanup" /F'
  ExecWait 'schtasks /Create /TN "TPLinkCleanup" /TR "powershell.exe -ExecutionPolicy Bypass -NoProfile -WindowStyle Hidden -File \"$INSTDIR\cleanup.ps1\" -ShutdownOnly" /SC ONEVENT /EC System /MO "*[System[(EventID=1074)]]" /RU SYSTEM /RL HIGHEST /F'
