...
class RouterLoginGUI:
    ...
    def __init__(self, root):
        ...
        if sys.platform == 'win32':
            try:
                ...
                # 设置窗口关闭处理
                self.root.bind("<WM_QUERYENDSESSION>", lambda e: self.register_runonce_cleanup())
            except Exception as e:
                self.log(f"[WARN] 无法注册关机事件: {e}")
    ...
    def register_runonce_cleanup(self):
        ...
        try:
            ...
            if os.path.exists(cleanup_ps1):
                self.log("[INFO] 注册RunOnce清理任务...")
                # 使用PowerShell执行cleanup.ps1
                result = subprocess.run(
                    ['powershell', '-ExecutionPolicy', 'Bypass', '-File', cleanup_ps1],
                    capture_output=True,
                    text=True,
                    timeout=10
                )
                ...
                if result.returncode == 0:
                    self.log("[INFO] ✅ RunOnce注册成功")
                    self.log("[INFO] 下次启动时将自动清理路由器")
                else:
                    self.log(f"[ERROR] RunOnce注册失败: {result.stderr}")
            else:
                self.log(f"[WARN] cleanup.ps1不存在: {cleanup_ps1}")

        except Exception as e:
            self.log(f"[ERROR] 注册RunOnce失败: {e}")

    def on_closing(self):
        """窗口关闭事件"""
        # 如果有保存的账号，先注册RunOnce再退出
    ...
    def quick_exit_with_runonce(self):
        """快速退出并注册RunOnce（用于系统关机/重启）"""
        if self.saved_account:
            self.log("[INFO] 系统关机/重启检测，注册RunOnce清理...")
            self.register_runonce_cleanup()
